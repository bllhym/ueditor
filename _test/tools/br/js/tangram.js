...
//	            if (!b.opera || parent.tagName != 'TR') {
    ...
    baidu.dom.query = (function () {
        ...
        var TPL_FIND = {
            ...
            'N': TPL_DOC + 'var #{N}A=doc.getElementsByName("#{P}");for(var #{N}I=0,#{N};#{N}=#{N}A[#{N}I];#{N}I++){if(#{R}===doc||' + format(TPL_CONTAINS, [
                '#{R}',
                '#{N}'
            ]) + '){#{X}}}',
        ...
        var TPL_HELP = '/*^var #{N}L;^*/if(!#{N}L||!(' + format(TPL_CONTAINS, [
            '#{N}L',
            '#{N}'
        ]) + ')){#{X}#{N}L=#{N};}';
