...
//        ok(this.equalsNode(ele.innerHMTL,html),descript);
    ...
    readTxt: function (name, f) {
        ...
        var nodeAStyle = this.removeEndSemicolon(nodeA.getAttr("style").replace(/\s+/g, ""))
                             .replace(/&quot;/g, '')
                             .split(";");
        var nodeBStyle = this.removeEndSemicolon(nodeB.getAttr("style").replace(/\s+/g, ""))
                             .replace(/&quot;/g, '')
                             .split(";");
        ...
        if (!(lengthA && lengthB)) {
            ...
            for (var p in nodeA.attrs) {
                if (!nodeB.getAttr(p) && !nodeA.getAttr(p)) {
                    ...
                    if (this.formHref(nodeA.getAttr(p).toLowerCase()) != this.formHref(nodeB.getAttr(p)
                                                                                            .toLowerCase())) {
                        return false;
                    }
                } else {
                    if (nodeA.getAttr(p).toLowerCase().replace(/^\s+|\s+$/g, "") != nodeB.getAttr(p)
                                                                                         .toLowerCase()
                                                                                         .replace(/^\s+|\s+$/g, "")) {
                        return false;
                    }
