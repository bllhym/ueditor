...
test('block的元素-样式的反复转换：块-内联；包含的选取方式：闭合、多段部分', function () {
    ...
    range.setStart(editor.body.getElementsByTagName('h1')[0].lastChild, 0)
         .setEnd(editor.body.getElementsByTagName('h1')[1].firstChild, 2)
         .select();//多段部分
    editor.execCommand('customstyle', {
        tag: 'span',
        label: '明显强调',
    ...
    setTimeout(function () {
        ...
        setTimeout(function () {
            ...
            range.setStart(editor.body.firstChild.firstChild.firstChild, 0)
                 .setEnd(editor.body.lastChild.firstChild, 3)
                 .select();
            editor.execCommand('customstyle', {
                tag: 'span',
                label: '明显强调',
