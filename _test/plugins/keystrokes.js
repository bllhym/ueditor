...
test('trace 3714跨节点输入tab键', function () {
    ...
    setTimeout(function () {
        range.setStart(editor.body.lastChild.firstChild, 0)
             .setEnd(editor.body.lastChild.firstChild.nextSibling, 1)
             .select();
...
test('在列表中，跨行选中第2，3行，输入tab键', function () {
    ...
    var range = te.obj[1];
    setTimeout(function () {
        range.setStart(editor.body.childNodes[0].childNodes[1].firstChild.firstChild, 1)
             .setEnd(editor.body.childNodes[0].childNodes[2].firstChild.firstChild, 1)
             .select();
        ua.keydown(editor.body, {'keyCode': 9});
        ua.keyup(editor.body, {'keyCode': 9});
        setTimeout(function () {
...
test('在列表中，跨行选中，输入tab键', function () {
    ...
    setTimeout(function () {
        range.setStart(editor.body.firstChild.firstChild.firstChild.firstChild, 1)
             .setEnd(editor.body.firstChild.childNodes[1].firstChild.firstChild, 1)
             .select();
        ua.keydown(editor.body, {'keyCode': 9});
        ua.keyup(editor.body, {'keyCode': 9});
        setTimeout(function () {
