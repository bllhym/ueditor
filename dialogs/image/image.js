...
(function () {
    ...
    function initButtons() {
        ...
        //抓取按钮
        domUtils.on($G('btn_catch'), 'click', function () {
            var url = $G('url').value;
    ...
    RemoteImage.prototype = {
        ...
        setImage: function (img) {
            ...
            var wordImgFlag = img.getAttribute("word_img"),
                src = wordImgFlag ? wordImgFlag.replace("&amp;", "&") : (img.getAttribute('_src') || img.getAttribute("src", 2)
                                                                                                        .replace("&amp;", "&")),
    ...
    function DoCatch(remoteImage) {
        ...
        if (/^(https?|ftp):/i.test(remoteImage) && !test(remoteImage, catcherLocalDomain)) {

            $G('btn_catch').innerText='抓取中...';
            $G('btn_catch').setAttribute("disabled", "disabled");

            catchremoteimage(remoteImage, {
                //成功抓取
                success: function (r) {
                    $G('btn_catch').innerText='抓取图片';
                    $G('btn_catch').removeAttribute("disabled");
                    ...
                    var cj=info.list[0],newSrc = catcherUrlPrefix + cj.url,oldSrc=remoteImage;
